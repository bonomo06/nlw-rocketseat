const formatador = (data) => {
  ...
  formatador(new Date('2024-04-05'))
  ...
  const atividade = {
    ...
    const criarItemAtiv = (atividade) => {
    
      let input = `
      <input
      onchange="concluirAtiv(event)"
    ...
    section.innerHTML += criarItemAtiv(atividade)
    }
  ...
  const salvarAtividade = (event) => {
    ...
    const DadosForms = new FormData(event.target)
  
    const nome = DadosForms.get('atividade')
    const dia = DadosForms.get('dia')
    const hora = DadosForms.get('hora')
    const data = `${dia} ${hora}`
    const atividade = {
      nome,
      data,
